...
export class ColumnsService {
  ...
  async getAllColumns(project:Project){
    return await this.columnsRepository.find({
      ...
      },project:{creater:true}}
    })
  }

  async deleteColumn(project:Project, param){
  ...
  async repozColumn(project:Project,param,pozition){
    const column=await this.getOneColumn(project,param)
    const length=await this.CountColumns(project)
  ...
  async getOneColumn(project:Project, param){
  ...
  async updateColumn(project:Project,param,columnDto){
    const column=await this.columnsRepository.findOne({
      where:{project:{id:project.id}, id:param.column_id}
    })
  ...
  private async CountColumns(project:Project){
    const listColumn=await this.columnsRepository.find({where:{project:{id:project.id}}})
