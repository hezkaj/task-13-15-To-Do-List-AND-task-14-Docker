...
export class ValuesService {
  ...
  async createValues(putField, value, task){
    if (putField.length){
      for (let field of putField){
        if(field.type=='string'){
          let stringDto:CreateStringFieldDto;
          stringDto = value[field.name] 
          await this.stringsRepository.save({value:value, task:{id:task.id}, field:{id:field.id}})
        }else if(field.type=='number'){
          let numberDto:CreateNumberFieldDto;
          numberDto = value[field.name] 
          await this.numbersRepository.save({value:value, task:{id:task.id}, field:{id:field.id}})
        } else if(field.type=='enum'){
          if(field.enum_array.includes(value[field.name])){
            let stringDto:CreateStringFieldDto;
            stringDto = value[field.name]
            await this.enumsRepository.save({value:value, task:{id:task.id}, field:{id:field.id}})
          }else{
  ...
  async updateValues(putField, value, task){
    if (putField.length){
      for (let field of putField){
        if(field.type=='string'){
          let stringDto:CreateStringFieldDto;
          ...
          await this.stringsRepository.save({value:value, task:{id:task.id}, field:{id:field.id}, id:Id.id})
        }else if(field.type=='number'){
          let numberDto:CreateNumberFieldDto;
          numberDto = value[field.name] 
          const Id= await this.getNumberId(task, field)
          await this.numbersRepository.save({value:value, task:{id:task.id}, field:{id:field.id}, id:Id.id})
        } else if(field.type=='enum'){
          if(field.enum_array.includes(value[field.name])){
            let stringDto:CreateStringFieldDto;
            stringDto = value[field.name]
            const Id= await this.getEnumId(task, field)
            await this.enumsRepository.save({value:value, task:{id:task.id}, field:{id:field.id}, id:Id.id})
          }else{
            throw new ValidationException({message:'В списке нет этого значения'})
          } 
        }  
  ...
  async getEnumId(task:Task, field:Field){
    const en= await this.enumsRepository.findOne({where:{task:{id:task.id}, field:{id:field.id}}})
    if(!en){return{id:1}}
    return en
  }
  async getStringId(task:Task, field:Field){
    const st= await this.stringsRepository.findOne({where:{task:{id:task.id}, field:{id:field.id}}})
    if(!st){return{id:1}}
    return st
  }
  async getNumberId(task:Task,field:Field ){
    let nu= await this.numbersRepository.findOne({where:{task:{id:task.id}, field:{id:field.id}}})
    if(!nu){return{id:1}}
    return nu
  }
  /*
